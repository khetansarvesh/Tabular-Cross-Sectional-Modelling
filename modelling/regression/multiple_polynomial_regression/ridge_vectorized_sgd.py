...
class StochasticGradientDescent_Ridge:
  ...
  def fit(self,X,Y,learning_rate = 0.000001,no_of_iteration = 10000,lambda_ridge = 0.01):
    ...
    for i in range(no_of_iteration + 1):
      ...
      self.new_theta = self.current_theta - (learning_rate)*((np.dot(self.Xp.transpose(),(np.dot(self.Xp,self.current_theta)-self.Yp.transpose()))) + 2*(lambda_ridge)*(self.current_theta))
