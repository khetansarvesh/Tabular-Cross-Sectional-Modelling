...
class GradientDescent_Lasso:
  ...
  def fit(self,X,Y,learning_rate = 0.000001,no_of_iteration = 10000,lambda_lasso=0.01):
    ...
    for i in range(no_of_iteration +1):
      ...
      self.new_theta = self.current_theta - (learning_rate)*((np.dot(self.X.transpose(),(np.dot(self.X,self.current_theta)-self.Y.transpose()))) + (lambda_lasso)*(np.sign(self.current_theta)))
