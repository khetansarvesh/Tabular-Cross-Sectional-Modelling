...
class StochasticGradientDescent_Lasso:
  ...
  #returns nothing
  def fit(self,X,Y,learning_rate = 0.000001,no_of_iteration = 10000,lambda_lasso = 0.01):
    ...
    for i in range(no_of_iteration + 1):
      ...
      self.new_theta = self.current_theta - (learning_rate)*((np.dot(self.Xp.transpose(),(np.dot(self.Xp,self.current_theta)-self.Yp.transpose()))) + (lambda_lasso)*(np.sign(self.current_theta)))
      ...
      self.error = ((np.dot(np.dot(self.X,self.new_theta.transpose()) - self.Y.transpose(),(np.dot(self.X,self.new_theta.transpose()) - self.Y.transpose()).transpose()))/2 + ((lambda_lasso)*(np.abs(self.new_theta))))
